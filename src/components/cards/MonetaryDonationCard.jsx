/***
 * This component is used as a template for the monetary donation card.
 * 
 * Is used in:
 * - 
 */
...
const MonetaryDonationCard = ({ image, startDate, endDate, title, description, progress, fundsReceived, targetFunds, donatePath, detailPath, sharePath, className, style, ...props }) => {
  return (
    <div
        ...
        <div className='w-full flex flex-col gap-2'>
          ...
          <div className='relative w-full h-[15px]'>
            <span
              ...
              style={{
                left: `calc(${fundsReceived / targetFunds * 100}% - 20px)`,
                color: PrimaryColor.red,
                top: 0,
                transition: 'left 0.3s',
              }}
            >
              {(fundsReceived / targetFunds * 100).toFixed(2)}%
            </span>
          </div>
          {/* Progress Bar */}
          <div className='w-full h-[10px] rounded-full' style={{ backgroundColor: SecondaryColor.light_grey }}>
            <div className='h-full rounded-full' style={{ width: `calc(${fundsReceived / targetFunds * 100}%)`, backgroundColor: PrimaryColor.red }}></div>
          </div>
          <Subtitle style={{ color: PrimaryColor.red }}>
            Dana terkumpul <strong>Rp {fundsReceived.toLocaleString('id-ID')}</strong> dari <strong>Rp {targetFunds.toLocaleString('id-ID')}</strong>
          </Subtitle>
        </div>
      </div>
